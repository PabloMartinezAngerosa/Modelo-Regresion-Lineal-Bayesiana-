...
# Analisis de Multicolinealidad, no hay valores >= 10 -> no hay multicolinealidad
car::vif(fittedGenearlLinearModel)

# Obtenemos modelo ajustados por step wise AIC
stepWiseFitted = stats::step(fittedGenearlLinearModel, direction= "both", trace=0)
...
stepWiseFittedInf = statas::lm(formula = MORT ~ PREC + JANT + JULT + POPN + EDUC + NONW + SO, data = datos[-c(6, 28, 32, 37),])
...
fittedModel <- stats::lm(formula = MORT ~ PREC + JANT + JULT + POPN + EDUC + NONW + SO , datos[-c(6, 28, 32, 37, 2),])
